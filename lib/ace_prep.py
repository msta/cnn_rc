...
class AcePrep(Preprocessor):
    ...
    def __init__(self, clipping_value=18, markup=False):
        self.label_dict = {}
        self.word_dict = {}
        Preprocessor.__init__(self)

    def load_dataset(self, debug=False):
        all_data = open("ace2005/training/ace2.csv").read().split("\n")
        self.word_dict = pickle.load(open("ace2005/training/vocab.pkl"))
        if debug:
            return all_data[:10]
        return all_data

    def read_dataset(self, debug=False):
        data = self.load_dataset(debug)
        ...
        for sample in data:
            ...
            self.nom_heads.append((splits[1], splits[2]))
    ...
    def fit_transform(self, data, labels):
        ...
        padded_data = self.pad_and_heads(data, self.nom_heads, self.clipping_value)
        nom_arr_1, nom_arr_2 = self.adjust_nominal_positions(padded_data, self.nom_heads)
        return (padded_data, 
            nom_arr_1, nom_arr_2, 
            None, None, labels)
