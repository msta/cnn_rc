...
def main(args):
   ...
   # with K.tf.device('/gpu:0'):
    ...
    nom_pos_1_full = nom_pos_1
    nom_pos_2_full = nom_pos_2

    for i in range(1, 21):

        cut = int((len(word_input)/100) * (i * 5))

        X_train = [word_input[:cut]]
        nom_pos_1 = nom_pos_1_full[:cut]
        nom_pos_2 = nom_pos_2_full[:cut]
     ...
     #   X_train = [np.append(word_input, word_input_aug, axis=0)]
      ...
      #  nom_pos_1 = np.append(nom_pos_1, nom_pos_aug_1, axis=0)
       # nom_pos_2 = np.append(nom_pos_2, nom_pos_aug_2, axis=0)
        ...
        with open("data/semeval/test_pred" + str(i) + ".txt", "w+") as f:
